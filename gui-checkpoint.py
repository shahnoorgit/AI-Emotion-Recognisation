...
top.title('Emotion Detecter ML Model')
top.configure(background="#CDCDCD")

label1=  tk.Label(top,background='#CDCDCD',font=('arial',15,'bold'))
sign_image=tk.Label(top)

facec= cv2.CascadeClassifier('haarcascade.frontalface_default.xml')
...
def Detect(file_path):
  ...
  try:
    for (x,y,w,h) in faces:
     ...
     label1.configure(foreground="#011638",text="unable to detect this complex image")
...
heading=tk.Label(top,text="Emotion Detector ML",pady=20,font=('arial',25,'bold'))
